...
public class CustomDriver {
	...
	public void newTabWithURL(String URL) {
		...
		log.info("New Tab is generated with the URL.." + URL);
	...
	/**
	 * Click element with information about element and time to wait in seconds
	 * after click
	 *
	...
	public void elementJsClick(WebElement element, String info, long timeToWait) {
		try {
	 ...
	 * 
	 * @param locator    - locator strategy, id=>example, name=>example,
	 *                   css=>#example, * tag=>example, xpath=>//example,
	 *                   link=>example
	 * @param info
	 * @param timeToWait
	 * @return
	...
	public void elementJsClick(String locator, String info, long timeToWait) {
		WebElement element = this.getElement(locator, info);
		elementJsClick(element, info, timeToWait);
	}
	...
	/**
	 * Waiting for max timeout seconds for element to be available
	 * 
	 * @param locator
	 * @param timeout
	 * @return
	...
	/**
	 * Waiting for max time for element to be available
	 */
	...
	public void elementClickNewWindow(String locator, int timeout, String info) {

		originalWindow = driver.getWindowHandle();

		// getElement(locator, info).click();
		elementJsClick(locator, info,3);

		waitForNumberOfWindowsToBeTwo(timeout);

	}

	public void waitForNumberOfWindowsToBeTwo(int timeout) {

		try {
			driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(0));
			log.info("Waiting for max:: " + timeout + " seconds for window to be appeared");
			...
			for (String windowHandle : driver.getWindowHandles()) {
				if (!originalWindow.contentEquals(windowHandle)) {
					...
					log.info("New Window successfully retrieved");
					break;
				}
			}
			...
			log.error("New window is not comming");
			driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(3));
		}
	...
	public String getTextFromInputField(String locator, String info) {
		String text = null;
		...
		try {
			...
			log.info("Got the text from the input  " + text);

		} catch (Exception e) {
			log.error("Cannot find the text");
		}
